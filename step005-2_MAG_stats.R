...
mp2<-autoplot.OpenStreetMap(openproj(openmap(c(35.00,-15.56),c(69.16,30),type='apple-iphoto',zoom=5)))+
  geom_point(data = meta_data,aes(x=longitude,y=latitude,col=temperature,size=pH1))
mp3<-autoplot.OpenStreetMap(openproj(openmap(c(35.00,-15.56),c(69.16,30),type='osm',zoom=5)))+
  geom_point(data = meta_data,aes(x=longitude,y=latitude,col=temperature,size=pH1))
mp+labs(colour="Temperature",size="pH",title="Sampling Locations With pH, Temperature, and Elevation")+
...
kosummary_table<-as.data.frame(fread("MAGs/outfile/clusters_dastool_90_10_average/gene_differential_presence.tsv"))
...
out<-pathview(gene.data=kosummary_table[,1:3],
              ...
              high = list(gene = "#332288", cpd = "#332288"),
              mid=list(gene = "#88CCEE", cpd = "#44AA99"),
